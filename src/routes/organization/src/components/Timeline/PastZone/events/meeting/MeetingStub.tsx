  ...
  return (
    <Card
      ...
      className={cn(
        ...
        'bg-white max-w-[549px] border border-grayModern-200 rounded-lg cursor-pointer shadow-none hover:shadow-sm transition-all duration-200 ease-out',
      )}
    >
      <CardContent className='px-3 py-2'>
        <div className='flex w-full justify-between relative gap-3'>
          <div className='flex flex-col items-start max-w-[461px]'>
            <p className='text-sm font-semibold text-grayModern-700 line-clamp-1'>
              {data?.name ?? '(No title)'}
            </p>
            <div className='flex'>
              <p
                className={cn(
                  ...
                  'text-sm text-grayModern-700 max-w-[463px]',
                )}
              >
                {owner || firstParticipant}{' '}
                <span className='text-grayModern-500'>met</span> {participants}
              </p>
              {remaining && (
                <span className='text-sm text-grayModern-500 ml-1 whitespace-nowrap'>
                  {` + ${remaining}`}
                </span>
              )}
            </div>
            ...
            {(note || agenda) && (
              <div className='flex items-start max-w-[517px]'>
                {note && (
                  <File02 className='size-3 mt-1 mr-1 text-grayModern-500' />
                )}
                <p className='text-sm text-grayModern-500 line-clamp-2'>
                  {note || agenda}
                </p>
              </div>
            )}
          ...
          <div className='flex min-w-[48px] h-[40px] text-3xl items-center justify-center'>
            <MeetingIcon />
            <p className='absolute text-xl font-semibold text-grayModern-700'>
